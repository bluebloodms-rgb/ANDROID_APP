...
## 🔧 Update — 2026-08-26 (هـ): اتصال به فلایت‌کنترلر ArduPilot — پارس MAVLink برقرار شد

### کرش‌ها و باگ‌هایی که با دیباگ زنده adb پیدا و رفع شدند

1. **پارسر فریم غیراستاندارد بود**: طول ۲ بایتی فرض شده بود؛ MAVLink v2 واقعی طول ۱ بایتی +
   هدر ۱۰ بایته دارد → بازنویسی کامل encode/parse طبق استاندارد
2. **CRC_EXTRA دو بایته جمع می‌شد**: باید فقط یک بایت باشد! (باگ متقارن در انکودر باعث شده بود
   تست‌های رندتریپ قبلی پاس شوند و باگ پنهان بماند)
3. **جدول CRC_EXTRA ناقص**: فقط ۷ پیام داشت؛ جدول رسمی ۲۳۴ پیام common + ۷۳ پیام
   ArduPilotMega (۳۰۷ کل) با اجرای خود ژنراتور رسمی pymavlink تولید شد
4. **فریم‌های بین‌چانکی**: فریم‌هایی که مرز چانک ۶۴ بایتی بلوتوث را قطع می‌کردند دور ریخته می‌شدند
   → بافر انباشت ماندگار با resync تک‌بایتی پیاده شد
5. **شناسه‌های پیام اشتباه**: RANGEFINDER در ArduPilot = 173 نه 174؛ آفست‌های GPS_RAW_INT
   اصلاح شد (eph@21, satellites@29)؛ SYS_STATUS(1) برای ولتاژ باتری اضافه شد

### شناسایی سخت‌افزار (از روی لاگ زنده)
- دستگاه متصل «SIYI-XXXX» در واقع فلایت‌کنترلر با فریمور **ArduPilot** است
  (پیام‌های ATTITUDE/GPS_RAW_INT/RC_CHANNELS/AHRS2/EKF_STATUS_REPORT/MEMINFO استریم می‌شود)
- پیام‌های دریافتی تأییدشده: HEARTBEAT(0), SYS_STATUS(1), SYSTEM_TIME(2), PING(4),
  GPS_RAW_INT(24), ATTITUDE(30), GLOBAL_POSITION_INT(33), RC_CHANNELS(65),
  RANGEFINDER(173), MAG_CAL_REPORT(136), MEMINFO(152), AHRS(163), AHRS2(178), EKF_STATUS_REPORT(193)

### وضعیت
- نرخ پارس موفق ≈۹۵٪+ ؛ خطاهای باقی‌مانده نویز مرز چانک است
- تلمتری BAT/SAT/ALT/HDOP حالا از داده واقعی پر می‌شود
- START/CANCEL به‌صورت STATUSTEXT ارسال می‌شود — ⚠️ ArduPilot این پروتکل سفارشی را
  نمی‌فهمد؛ برای کنترل واقعی باید به COMMAND_LONG (MAV_CMD_DO_SET_MODE و ...) مهاجرت کرد
